...
updateAnchorPeer1(){
    setGlobalsForPeer0Org1
    peer channel update -o localhost:7050 -c mychannel -f ./artifact/Org1Anchor.tx --tls --cafile ${PWD}/crypto-config/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem
}

updateAnchorPeer2(){
    setGlobalsForPeer0Org2
    peer channel update -o localhost:7050 -c mychannel -f ./artifact/Org2Anchor.tx --tls --cafile ${PWD}/crypto-config/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem
}
